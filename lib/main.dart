...
void main() {
  ...
  // MyApp은 const로 선언했으면 불변 객체임을 의미 - 성능 최적화 목적
}
...
// StatelessWidget : 상태를 가지지 않는 위젯. 한 번 앱에 표시(렌더링)되면, 절대 변경되지 않는 UI들로 구성
/*위젯 (태그 + css + js)
      : UI를 구성하는 기본요소이자 단위
      눈에 보이는 요소 뿐 아니라 눈에 보이지 않음에도 레이아웃이나 그외 특수한 기능을 담당하는 것도 위젯에 포함
...
/*    상태가 있는 위젯 - 가변 특성을 가짐 - UI의 상태에 따라 상황이 바뀌는 위젯
      즉, 상태가 변경되면 UI를 다시 렌더링 할 수 있음 - build 다시 호출함
      위젯 내부에서 build 함수 재실행해야 할 필요가 있을때 사용*/

// stateful
// 어떻게 보이게 할 건지에 영향을 미치는 필드를 포함하는 state 객체를 의미
...
// 플러터앱의 상태 관리와 UI 구현에 대한 내용 정리
class _MyHomePageState extends State<MyHomePage> {
  int _counter = 0; // 상태 변수 정의

  void _incrementCounter() {  // 상태를 변화시키는 메서드 정의
      setState(() {
      // setState를 이용해서 플러터에게 상태 변화가 생김을 알림
      // 이를 통해 변경된 state를 UI에 반영하기 위해 build 메서드 재실행함.
      // setState 호출 없이 _counter 변수의 상태를 변경하는 경우에는
      // build 메서드는 재실행하지 않음
      _counter++;
    });
  }

  void _decrementCounter() {  // 상태를 변화시키는 메서드 정의
    setState(() {
      _counter--;
    });
  }
  ...
  @override
  // state에 따라 위젯의 UI를 렌더링하는 메서드
  Widget build(BuildContext context) {
    // build : setState가 호출되었을 때 재실행되는 메서드
    // 위의 예에서는 _incrementCounter가 호출되어 상태 변화가 생겼을 때의 build 재실행
    // 플러터는 이러한 build를 재실행하는 데 최적화되어 있음
    ...
    return Scaffold(
      ...
      body: Center(
        // Center는 레이아웃 위젯 - 자식요소를 받아 중앙에 배치

        child: Column(
          // Column도 레이아웃 위젯 - 자식요소를 받아 수직으로 배치
          // Column 위젯에는 다양한 속성 존재
          // 각 위젯의 와이어프레임을 보려면 콘솔에서 p를 입력
